...
.officer-badge{font-size:10.5px;font-weight:700;padding:2px 8px;border-radius:100px;background:#1A4BAD;color:#fff;}
.officer-reason{
...
/* ── Vehicle type card ──────────────────────────────────────── */
.vehicle-cards{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin:24px 0;}
.vehicle-card{
    border:2px solid #EBEEF5;border-radius:16px;padding:28px 20px;
    text-align:center;cursor:pointer;background:#fff;
    transition:border-color .2s,box-shadow .2s,transform .15s;
    display:flex;flex-direction:column;align-items:center;gap:12px;
}
.vehicle-card:hover{border-color:#3B6FD4;box-shadow:0 4px 20px rgba(26,75,173,.12);transform:translateY(-2px);}
.vehicle-card.selected{border-color:#1A4BAD;background:#EEF3FC;box-shadow:0 4px 20px rgba(26,75,173,.15);}
.vehicle-icon{width:64px;height:64px;border-radius:50%;display:flex;align-items:center;justify-content:center;}
.vehicle-card-label{font-family:'Syne',sans-serif;font-size:1.1rem;font-weight:800;color:#181D35;}
.vehicle-card-sub{font-size:12px;color:#8A93AE;}
...
<div class="card" style="padding:0;overflow:hidden;border-radius:14px;border:1.5px solid #EBEEF5;box-shadow:0 1px 4px rgba(0,0,0,.06);">
    ...
    {{-- ══════════════════════════
         S-Tamu · Pilih jenis kendaraan (khusus tamu)
    ══════════════════════════ --}}
    <div class="kscreen" id="sTamu">
        <div class="step-bar">
        ...
        <div style="text-align:center;margin-bottom:6px;">
            <div class="screen-title">Pilih Jenis Kendaraan</div>
            <div class="screen-sub">Plat tidak terdaftar — pilih jenis kendaraan Anda untuk melanjutkan</div>
        </div>

        {{-- Info plat tamu --}}
        <div style="background:#fff;border:1.5px solid #EBEEF5;border-radius:9px;padding:10px 16px;margin-bottom:20px;text-align:center;">
            <div style="font-size:11px;color:#8A93AE;margin-bottom:4px;">Plat Nomor</div>
            <div id="tamuPlat" style="font-family:monospace;font-size:20px;font-weight:700;color:#181D35;letter-spacing:.1em;"></div>
            <span class="badge-yellow" style="margin-top:6px;display:inline-flex;">Tamu / Tidak Terdaftar</span>
        </div>

        <div class="vehicle-cards">
            {{-- Mobil --}}
            <div class="vehicle-card" id="tamuCardMobil" onclick="selectTamuVehicle('mobil')">
                <div class="vehicle-icon" style="background:#E8F0FB;">
                    <svg viewBox="0 0 24 24" fill="none" stroke="#1A4BAD" stroke-width="1.8" width="34" height="34">
                        <rect x="2" y="8" width="20" height="10" rx="2"/>
                        <path d="M5 8l2-4h10l2 4"/>
                        <circle cx="7" cy="18" r="2"/><circle cx="17" cy="18" r="2"/>
                    </svg>
                </div>
                <div class="vehicle-card-label">Mobil</div>
                <div class="vehicle-card-sub">Kendaraan roda empat</div>
            </div>

            {{-- Motor --}}
            <div class="vehicle-card" id="tamuCardMotor" onclick="selectTamuVehicle('motor')">
                <div class="vehicle-icon" style="background:#ECFDF3;">
                    <svg viewBox="0 0 24 24" fill="none" stroke="#027A48" stroke-width="1.8" width="34" height="34">
                        <circle cx="5.5" cy="17.5" r="2.5"/><circle cx="18.5" cy="17.5" r="2.5"/>
                        <path d="M8 17.5h7M8 17.5L10 10h4l3 4.5M10 10l-2-4h5"/>
                    </svg>
                </div>
                <div class="vehicle-card-label">Motor</div>
                <div class="vehicle-card-sub">Kendaraan roda dua</div>
            </div>
        </div>
        ...
        <div class="notice-box">
            ...
            <span>Konfirmasi petugas tetap diperlukan setelah memilih jenis kendaraan</span>
        </div>
    </div>
    ...
    <div class="kscreen" id="sOfficer">
        ...
        <div class="officer-box">
            ...
            <div style="display:flex;flex-wrap:wrap;gap:10px;margin-top:10px;">
                ...
                <div style="flex:1;min-width:130px;background:#F5F7FC;border-radius:8px;padding:9px 12px;">
                    <div style="font-size:11px;color:#8A93AE;margin-bottom:2px;">Jenis Kendaraan</div>
                    <div id="officerJenis" style="font-size:13px;font-weight:600;color:#181D35;"></div>
                </div>
                <div style="flex:1;min-width:130px;background:#F5F7FC;border-radius:8px;padding:9px 12px;">
...
let faceRetry       = 0;
let plateRetry      = 0;
let tamuVehicleType = null; // 'mobil' | 'motor' — khusus tamu
const MAX_RETRY     = 3;
...
function validatePlate(raw){ return PLATE_RE.test(raw.replace(/\s+/g,'').toUpperCase()); }
...
// ── Tamu: pilih jenis kendaraan ────────────────────────────────
function openTamuVehicleSelect(data){
    tamuVehicleType = null;
    document.getElementById('tamuPlat').textContent = data.plat;
    document.getElementById('tamuCardMobil').classList.remove('selected');
    document.getElementById('tamuCardMotor').classList.remove('selected');
    foundData = data;
    showScreen('sTamu');
}

function selectTamuVehicle(type){
    tamuVehicleType = type;
    document.getElementById('tamuCardMobil').classList.toggle('selected', type==='mobil');
    document.getElementById('tamuCardMotor').classList.toggle('selected', type==='motor');

    const jenisLabel = type === 'mobil' ? '🚗 Mobil' : '🏍 Motor';

    // Langsung ke officer dengan info jenis kendaraan
    setTimeout(()=>{
        goOfficer(
            foundData.plat,
            foundData.aksi ?? 'masuk',
            'tamu-not-found',
            `Plat <strong>${foundData.plat}</strong> tidak ditemukan dalam database setelah ${MAX_RETRY} percobaan. Jenis kendaraan: <strong>${jenisLabel}</strong>.`
        );
    }, 150);
}
...
function submitPlate(){
    ...
    .then(r=>r.json())
    .then(data=>{ foundData = data; processResult(data); })
...
function processResult(data){
    ...
    if(data.status==='tamu'){
        if(aksi === 'keluar'){
            // Tamu keluar → tidak perlu pilih kendaraan, langsung officer
            goOfficer(
                data.plat, aksi, 'tamu-not-found',
            ...
            if(plateRetry >= MAX_RETRY){
                // Habis retry → minta pilih jenis kendaraan dulu
                openTamuVehicleSelect({...data, aksi});
            } else {
                showScreen('s1');
                const inp = document.getElementById('plateInput');
                inp.value = data.plat;
...
function openFaceScan(data){
    ...
    faceTimeout = setTimeout(()=>{
        goOfficer(
            data.plat, data.aksi??'masuk', 'no-face',
...
function doFaceScan(showLoad=true){
    ...
    .then(r=>r.json())
    .then(data=>{
        if(data.status==='waiting'){ setTimeout(()=>doFaceScan(false), 1000); return; }
...
function handleFaceResult(isMatch){
    ...
    if(isMatch){ confirmAction(); return; }
    faceRetry++;
    updateFaceRetryBar();
    if(faceRetry>=MAX_RETRY){
        goOfficer(
            foundData.plat, foundData.aksi??'masuk', 'face-fail',
...
function goOfficer(plat, aksi, type, reason, data=null){
    ...
    const jenisLabel = tamuVehicleType === 'mobil' ? '🚗 Mobil'
                     : tamuVehicleType === 'motor' ? '🏍 Motor'
                     : '-';

    document.getElementById('officerPlat').textContent   = plat;
    document.getElementById('officerReason').innerHTML   = reason;
    document.getElementById('officerJenis').textContent  = jenisLabel;
...
function resetKiosk(){
    ...
    foundData=null; faceVerified=null; faceRetry=0; plateRetry=0;
    tamuVehicleType=null; officerReason=''; isConfirming=false;
